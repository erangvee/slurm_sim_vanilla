...
static double _get_priority(priority_factors_object_t *prio_factors)
{
	...
	info("GET_PRIORITY: priority_wra=%d", prio_factors->priority_wra);
...
int print_job_from_format(priority_factors_object_t * job, List list)
{
	...
	info("TOTAL_WIDTH=%d", total_width);
...
{
   	if (job == NULL)   /* Print the Header instead */
	       	_print_str("WRA", width, right, true);
    	else if (job == (priority_factors_object_t *) -1)
        	_print_int(weight_wra, width, right, true);
    	else {
        	double num = 0;
	        if (weight_wra)
        	    	num = job->priority_wra / weight_wra;
        		_print_norm(num, width, right, true);
    	}	
               ...
               bool right, char* suffix)
{
  	if (job == NULL)    /* Print the Header instead */
	       	...
	       	_print_int(job->priority_wra, width, right, true);
   	if (suffix)
        	printf("%s", suffix);
    	return SLURM_SUCCESS;
}
