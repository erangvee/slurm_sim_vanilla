...
void sim_sprio()
{
	...
	fprintf(fout, "          JOBID     USER   PRIORITY        AGE  FAIRSHARE    JOBSIZE  PARTITION        QOS        WRA         NICE                 TRES\n");
	...
	if(priority_factors_list!=NULL){
		...
		while ((job = list_next(itr))) {
			...
			fprintf(fout,"%15u %8s %10.0f %10.0f %10.0f %10.0f %10.0f %10.0f %10.0f %11ld %20s\n",
					job->job_id,
...
void sim_squeue()
{
	...
	while ((job = (struct job_record *) list_next(job_iterator))) {
		...
		if (!IS_JOB_COMPLETE(job)){
        	fprintf(fout, "%18u %9s %8s %8s", job->job_id,job->partition,
							job->name,uid_to_string_cached(job->user_id));
                time_t end = job->end_time;
		char str[32];
                _sim_print_secs(str,now-job->start_time,10,1,0);
			fprintf(fout," %2s %10s %6d %10s %15u\n", job_state_string_compact(job->job_state),str,job->node_cnt,job->nodes,(end-now));
		if (end-now < 60) {
			if (604800 >= now-job->start_time >= 0){
			//job->end_time=end+3600;
                         job->end_time = end + (int)(15 * pow(2, (i_iterate-2)))*60;
                         i_iterate++;
