...
#include "src/plugins/jobcomp/filesacctout/filesacctout_jobcomp_process.h"
...
static uint32_t weight_wra;  /* weight for WRA factor */
...
/* Sorts list based on an index */

static int _sort_by_job_id_desc(void *void1, void * void2)
{
    time_t start1 = (*(jobcomp_job_rec_t **)void1)->start_time;
    time_t start2 = (*(jobcomp_job_rec_t **)void2)->start_time;
    info("%u", start1);
    if (start1 > start2) {
        return -1;
...
/* Compute for walltime request accuracy priority */
// a number of info() were added for debugging purposes.
static double _get_walltime_request_accuracy_priority(struct job_record *job_ptr)
{
	double priority_wra = 1.0; 

	// get previous jobs of the user
	slurmdb_job_cond_t *job_cond = xmalloc(sizeof(slurmdb_job_cond_t));
	info("wra: assigning memory to job_cond");

	// set user_id list
	job_cond->userid_list = list_create(slurm_destroy_char);
	info("wra: assigning an empty list for job_cond->userid_list done.");

	/* 
 * 	 The following will produce a warning:
 * 	 slurm_addto_char_list(job_cond->userid_list, job_ptr->user_id);
 *	 and will hence lead to incomplete simulation (for some reason, it stops after adding to char_list.
 *
 * 	 To solve that, cast job_ptr->user_id to char, as slurm_addto_char_list only accepts
 * 	 strings. 
 * 	 */	
	
	char u_id[50];
	sprintf(u_id, "%u", job_ptr->user_id);
	info("wra: converted int user_id %u to char %s", job_ptr->user_id, u_id);
	
	slurm_addto_char_list(job_cond->userid_list, u_id);
	info("wra: added data to job_cond for jobid %u", job_ptr->job_id);

	/* limit runtime to specified time:
 * 	1. determine the submit time of the user. use this as reference time.
 * 	2. calculate the datetime limit for which the query is carried out.
 * 	3. add the datetime to the query.
 * 	4. perform the query.
 * 	5. get the last n jobs.
 * 	*/

	// get the submit time of the user and use it later to query jobcomp
	time_t submit_time;
	time_t time_window = 604800; // hardcoded for now. 7 days = 604800 seconds

	submit_time = job_ptr->details->submit_time;

	// get jobs where the usage start n days before this job was submitted
	job_cond->usage_start = submit_time - time_window;
		
	// get previous jobs of the user
	List prev_jobs = g_slurm_jobcomp_get_jobs(job_cond);
	info("wra: prev_jobs created");	

	// include a check if prev_jobs is an empty list. Set priority_wra to 1.0 if empty.
	if (list_is_empty(prev_jobs)) {
		info("wra: prev_jobs query for user %u is empty.", job_ptr->user_id);
		priority_wra = 1.0;
		info("wra: empty prev_jobs query. assigned priority_wra: %lf", priority_wra);
	} else {
		info("wra: prev_jobs length for user %u is %d", job_ptr->user_id, list_count(prev_jobs));
			
		// loop last n jobs
		uint n = 2; //hardcoded for now
		uint job_count = 0;
		float job_wras = 0.0;
		ListIterator itr = NULL;
		jobcomp_job_rec_t *job = NULL;

		// reverse sort list first based on job id
		list_sort(prev_jobs, _sort_by_job_id_desc);

		itr = list_iterator_create(prev_jobs);

		// if timelimit is INFINITE, set priority_wra to 0.0
	        if (job_ptr->time_limit == INFINITE) {
			info("wra: job_timelimit is INFINITE");
        	        priority_wra = 0.0;
	        } else {
			// loop through previous jobs
			while ((job = list_next(itr))){										
				// get out of loop if n is reached
				job_count = job_count + 1;
				
				if (!job){
					info("wra: NULL job");
				}
				info("wra: job_count %d", job_count);
				if (job_count > n) {
					info("wra: job_count %u is now greater than n %u", job_count, n);
					break;
				} else {
					info("wra: job_count = %d", job_count);
                                	float job_elapsed = (float)job->elapsed_time;
                        	        float job_timelimit = 60*(atof(job->timelimit)); // job_timelimit = 60*(atof(job->timelimit));
                	                info("wra: job_elapsed %lf and job_timelimit %lf", job_elapsed, job_timelimit);
        	                        job_wras = job_wras + (job_elapsed / job_timelimit);
	                                info("wra: jobid %u job_wra %10.5f job_start_time %s", job->jobid, job_elapsed / job_timelimit, job->start_time);
				}                          
			}
			
			/* compute for WRA index of previous n jobs. The function can by eanythin from just
	 * 		a simple average to more complex functions 
	 * 		*/
			info("wra: total job_wras = %lf", job_wras);
			priority_wra = job_wras / ((double) job_count-1);
			info("wra: priority_wra is %lf at n=%d", priority_wra, job_count-1);			
		}	 
		list_iterator_destroy(itr);
	}	
	// destroy
	FREE_NULL_LIST(prev_jobs);
				       ...
				       struct job_record *job_ptr)
{
	...
	job_ptr->prio_factors->priority_wra  *= (double)weight_wra;
	...
	priority = job_ptr->prio_factors->priority_age
		...
		+ job_ptr->prio_factors->priority_wra
		+ tmp_tres
	...
	if (priority_debug) {
		...
		info("Weighted QOS priority is %f * %u = %.2f",
		     ...
		     job_ptr->prio_factors->priority_qos);
		info("Weighted WRA priority is %f * %u = %.2f",
		     pre_factors.priority_wra, weight_wra,
		     job_ptr->prio_factors->priority_wra);
...
static void _internal_setup(void)
{
	...
	weight_wra = slurm_get_priority_weight_wra();
	xfree(weight_tres);
	...
	if (priority_debug) {
		...
		info("priority: Weight WRA is %u", weight_wra);
...
/*  Add walltime incentive here. Things to note
 *  1) where does one get the qos_ptr->priority?
 *  2) create a function similar to _get_fairshare_priority
 *   3) a sample is given below
 *  */
	if (weight_wra) {
           job_ptr->prio_factors->priority_wra =
               _get_walltime_request_accuracy_priority(job_ptr);
	}
