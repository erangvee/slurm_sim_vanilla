...
static uint32_t weight_wra;  /* weight for Walltime request accuracy factor */
...
{
/*
    char start1 = (*(jobcomp_job_rec_t **)void1)->start_time;
    char start2 = (*(jobcomp_job_rec_t **)void2)->start_time;
    ...
    info("%s %s", start1, start2);
    int diff;
    diff = strcmp(start1, start2); // int diff = xstrcmp(start1, start2);
    return -diff;
*/
...
//   double priority_wra = 1.0;
    /* IMPORTANT NOTE: 
        Some modules must be loaded in this script for this function to work.
        Do this later when the algorithm in this function is done.
    */

    // get previous jobs of the use
    slurmdb_job_cond_t *job_cond = xmalloc(sizeof(slurmdb_job_cond_t));
    info("wra: TEST assigning memory to job_cond");
    ...
    /* limit runtime to a specified time.
    steps:
        1. determine the submit time of the user. Use this as the reference time
    ...
    */
    ...
    //info("wra: set job_cond->usage_start to %u", submit_time - time_window);
    //info("wra: job_cond->usage_start is %u", job_cond->usage_start);

    // get previous jobs of user
    List prev_jobs = g_slurm_jobcomp_get_jobs(job_cond);

    // maybe include a check if prev_jobs is an empty list. Return if empty
    if (list_is_empty(prev_jobs) == 0)
        info("wra: prev_jobs query for jobid %u is empty.", job_ptr->job_id);

    info("wra: TEST prev jobs length for jobid %u is %d", job_ptr->job_id, list_count(prev_jobs));
    
    // loop last n jobs
    uint n = 5; // hardcoded for now
    uint job_count = 0;
    float job_wras = 0;
    ...
    while ((job = list_next(itr))) {
    	// get out if loop if n is reached.
        if (job_count == n)
	    break;
	...
	// compute for a jobs WRA (right now it will just be a simple average)
        float job_elapsed = (float)job->elapsed_time;
        float job_timelimit = atof(job->timelimit);// double job_timelimit = 60*(atof(job->timelimit)); // job->timelimit is in minutes
        
 	info("wra: jobid %u wra %10.5f job_start_time %s", job->jobid, job_elapsed/job_timelimit, job->start_time);
        job_wras = job_wras + (job_elapsed / job_timelimit); // job_wras = job_wras + 1.0; //(job_elapsed / job_timelimit);

    }
 
    /* compute for WRA index of previous n jobs. The funciton can be anything
    from just a simple average to more complex functions e.g. assigning more
    weights to more recent jobs
    */
    // set priority based on average WRA
    priority_wra = job_wras / n;

    // destroy
    list_iterator_destroy(itr);
    FREE_NULL_LIST(prev_jobs); // not sure about this yet
				       ...
				       struct job_record *job_ptr)
{
	...
	if (priority_debug) {
		...
		info("Weighted QOS priority is %f * %u = %.2f",
		     ...
		     job_ptr->prio_factors->priority_qos);
                info("Weighted Walltime request accuracy priority is %f * %u = %.2f",
		...
		info("Job %u priority: %.2f + %.2f + %.2f + %.2f + %.2f + %2.f + %2.f "
