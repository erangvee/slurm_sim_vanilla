...
//					info("wra: job_count = %d", job_count);
				...
				//
					if (job_elapsed > job_timelimit) {
						job_wras = job_wras;
					} else {
						float ans = job_elapsed / job_timelimit;
						if (ans != ans) {
							job_wras = job_wras;
						} else {
							job_wras = job_wras + ans;
						}
					}
				}
			}
...
//			info("wra: total job_wras = %lf", job_wras);
			priority_wra = job_wras / ((double) job_count);
			info("wra: priority_wra is %lf at n=%d", priority_wra, job_count);			
		}	 
		list_iterator_destroy(itr);
