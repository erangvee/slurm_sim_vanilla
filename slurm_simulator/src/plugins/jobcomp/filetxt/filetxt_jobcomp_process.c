...
static jobcomp_job_rec_t *_parse_line(List job_info_list)
{
	...
	//	info("job_comp: jobcomp_info->name %s", jobcomp_info->name);
		if (!xstrcasecmp("JobID", jobcomp_info->name)) {
			job->jobid = atoi(jobcomp_info->val);
			//info("_parse jobid %d", job->jobid);
			...
			//info("_parse timelimit %s", jobcomp_info->val);
...
extern List filetxt_jobcomp_process_get_jobs(slurmdb_job_cond_t *job_cond)
{
	...
	while (fgets(line, BUFFER_SIZE, fd)) {
		...
		//info("_fjpg: checking step_list ...");
		if (job_cond->step_list && list_count(job_cond->step_list)) {
			//info("_fjpg: entered at job_cond step list condition");
			if (!jobid)
				continue;
	...
	foundjob:
		//info("_fjpg: reached partiion list checking ..");
		if (job_cond->partition_list
		    && list_count(job_cond->partition_list)) {
			//info("_fjpg: entered at job_cond partition list");
			if (!partition)
				continue;
	...
	foundp:
		//info("_fjpg: for some reason I got here");
