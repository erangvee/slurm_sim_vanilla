	...
	while((jobcomp_info = list_next(itr))) {
		if (!xstrcasecmp("JobId", jobcomp_info->name)) {
			...
			error("Unknown type %s: %s", jobcomp_info->name,
			      jobcomp_info->val);
//			info("jobcomp_info val: %s", jobcomp_info->val);
		}
	}
...
extern List filesacctout_jobcomp_process_get_jobs(slurmdb_job_cond_t *job_cond)
{
    ...
    char * tmp = calloc(strlen(fptr)+1, sizeof(char));
    strcpy(tmp, fptr);
    ...
    { 
        ...
        if (i == 21) {
            char val[100];
            int i = isCharacterExist(token, '-', 0);
            if (i != -1) {
                  int day=0,hour=0,min=0,sec=0;
                  int elapsed_sec;
                  char str_t[100];
                  strcpy(str_t, token);
                  sscanf(str_t, "%d-%d:%d:%d", &day, &hour, &min, &sec);
                  elapsed_sec = (86400*day + 3600*hour + 60*min + sec) / 60;
                  sprintf(val, "%d", elapsed_sec);
            } else {
                  int hour=0,min=0,sec=0;
                  int elapsed_sec;
                  char str_t[100];
                  strcpy(str_t, token);
                  sscanf(str_t, "d:%d:%d", &hour, &min, &sec);
                  elapsed_sec = (3600*hour + 60*min + sec) / 60;
                  sprintf(val, "%d", elapsed_sec);
            }
            strcat(str, " Timelimit=");
            strcat(str, val);
	}
        if (i == 22) {
            strcat(str, " NodeList=");
            strcat(str, token);
            strcat(str, " WorkDir=/tmp");
            break;
        }
        i ++;
		...
		if (job_cond->step_list && list_count(job_cond->step_list)) {
			...
			while((selected_step = list_next(itr))) {
				if (selected_step->jobid == jobid)
					info("fptr: selected_step %u", selected_step->jobid);
					continue;
				/* job matches */
				list_iterator_destroy(itr);
				goto foundjob;
