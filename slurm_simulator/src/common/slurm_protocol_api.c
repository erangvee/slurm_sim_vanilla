...
/* slurm_get_wra_scale_factor
 * returns the scale factor for WRA from slurmctld_conf object
 * RET double - factor.
 */
/*double slurm_get_wra_scale_factor(void)
  {
    double factor = 1.0;
    slurm_ctl_conf_t *conf;
    if (slurmdbd_conf) {
...
/* slurm_set_wra_scale_factor
 * set the scale factor for WRA from slurmctld_conf object
 */
/* void slurm_set_wra_scale_factor(double factor)
 {
     slurm_ctl_conf_t *conf;
     ...
     if (slurmdbd_conf) {
         ...
         conf = slurm_conf_lock();
         conf->wra_scale_factor = factor;
         slurm_conf_unlock();
     }
 }
*/

/* slurm_get_priority_weight_wra
 * returns the priority weight for walltime request accuracy from slurmctld_conf object
 *  RET uint32_t - factor weight.
 */
uint32_t slurm_get_priority_weight_wra(void)
{
  uint32_t factor = NO_VAL;
  slurm_ctl_conf_t *conf;
   ...
   if (slurmdbd_conf) {
      ...
      conf = slurm_conf_lock();
      factor = conf->priority_weight_wra;
      slurm_conf_unlock();
   }
